/**
 * /api/analyze — 자연어 기록 → 구조화된 제안.
 *
 * API 키는 이 파일(서버 쪽)에서만 읽는다. 클라이언트 번들에는 들어가지 않는다.
 * - 배포: Vercel 서버리스 함수 (이 파일의 default export)
 * - 개발: server/http.ts 미들웨어가 아래 runAnalysis 를 그대로 가져다 쓴다.
 *
 * ⚠️ 이 파일에는 값(runtime) import 를 두지 않는다.
 *    package.json 이 "type": "module" 이라 함수는 ESM 으로 실행되는데,
 *    Node ESM 은 확장자 없는 상대 경로를 해석하지 못해 FUNCTION_INVOCATION_FAILED 로 죽는다.
 *    타입 import(컴파일 후 사라짐)만 사용한다.
 */
import type {
  AnalysisResult,
  AnalyzeRequest,
...
export default async function handler(req: Req, res: Res) {
  ...
  try {
    ...
    res.status(500).json({
      error: `분석 중 문제가 생겼습니다. (${(error as Error)?.message ?? 'unknown'})`,
      code: 'unknown',
    })
  }
}
